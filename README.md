...
5. **Data plane** 🚧 — Ecto-changeset structured output (`agentsea_structured`); an embeddings/vector-search stack with a RAG retrieval tool (`agentsea_embeddings`); and a Broadway document ingestion pipeline — chunk → embed → store with batching/backpressure (`agentsea_ingest`). Bumblebee/pgvector adapters and a Broadway evaluation pipeline next.
