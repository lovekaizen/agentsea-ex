...
5. **Data plane** 🚧 — Ecto-changeset structured output (`agentsea_structured`) and an embeddings/vector-search stack with a RAG retrieval tool — `Embedder`/`VectorStore` behaviours, a dependency-free hashing embedder, an in-memory cosine store, and `RetrievalTool` (`agentsea_embeddings`) — are done; Bumblebee/pgvector adapters and Broadway ingest/evaluate next.
