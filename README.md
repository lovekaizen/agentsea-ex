...
6. **Bridges** 🚧 — an MCP client (transport-agnostic handshake/list/call, with server tools adapted into agent `Tool.Spec`s) **and a real stdio transport** (spawns an MCP server subprocess, newline-delimited JSON-RPC, id-correlated) are done (`agentsea_mcp`); a streamable-HTTP MCP transport, a surf Node sidecar via Port, and voice next.
