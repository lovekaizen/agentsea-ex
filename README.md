...
5. **Data plane** 🚧 — Ecto-changeset structured output (`agentsea_structured`); an embeddings/vector-search stack with a RAG retrieval tool, an in-memory store, **and a pgvector store** (`agentsea_embeddings`); a Broadway document ingestion pipeline (`agentsea_ingest`); and concurrent evaluation with exact-match/contains/LLM-as-judge metrics + aggregation (`agentsea_evaluate`). A Bumblebee/Nx in-process embedder is the last remaining adapter.
